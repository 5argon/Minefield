...
## Why beacons, and how this compares to web UI testing
...
The web testing world has already had this exact debate, and the conclusions map onto Minefield almost one-to-one. Modern web frameworks like [Playwright](https://playwright.dev/docs/locators) and [Testing Library](https://testing-library.com/docs/queries/about/) rank the ways you can locate an element, worst to best:

- **CSS selectors / XPath / DOM paths** coupled to structure and styling. Both tools explicitly tell you to *avoid* these because "the DOM can often change, leading to non-resilient tests." This is the direct analog of `GameObject.Find("Canvas/Panel/Button")` or `transform.GetChild(2)` hierarchy-crawling in Unity. It couples to incidental implementation details that were never meant to be a test contract, and it breaks **silently** — the wrong object is found, or nothing is, with no error at author time.
- **Role / visible text** (`getByRole`, `getByText`) coupled to user-facing semantics. Playwright's *preferred* tier, because such a query also proves the element is actually reachable and accessible. The cost: it couples to copy, so renaming or localizing a label breaks the test (loudly, at least).
- **`data-testid`** — a deliberate, test-only attribute added to production markup so tests can find elements by a stable identity that survives restyles, restructures, and copy changes. Both tools treat this as the sanctioned escape hatch when role/text won't do.

**A Minefield beacon is `data-testid`.** The web ecosystem independently arrived at the same solution: when structural and semantic queries are too flaky, you *add explicit test metadata to the production artifact.* So the "it adds junk to my objects" critique isn't a Minefield wart — it's a well-understood, widely-accepted trade-off. Testing Library's own docs concede `data-testid` "adds extra complexity to component markup," and people use it anyway, because the alternative (structural coupling) is worse.

But an enum beacon is actually **better than `data-testid` on the axis that matters most**. `data-testid="training-button"` is a magic string on *both* sides — rename it in the markup, forget the test, and the test doesn't error; it silently fails to find the element. That is the *same failure mode as find-by-name* — the web tolerates it only because JavaScript has no compile step to catch it. A Minefield enum removes exactly that weakness: `ModeSelectScreen.Navigation.Training` is compile-checked on both the scene-authoring side (the `NavigationBeacon<>` field) and the test side. Delete or rename the member and **both fail to compile** — loud, immediate, un-ignorable.

Ranked by how a break surfaces:
...
Two honest counterpoints, so the trade-off is clear:

1. **Reachability.** `getByRole` fails when an element is invisible or not in the accessibility tree, so it doubles as an accessibility check. A bare beacon lookup bypasses the visual layer — an object can be found even if a real player couldn't see or hit it. Minefield recovers most of this through `Is.Clickable`, which does a real `EventSystem` raycast-first test plus `IsInteractable()`, and `ClickWhen(Is.Clickable)`, which retries every frame. That is conceptually identical to Playwright's **auto-waiting actionability** model (wait until the element is visible, stable, receives events, and enabled — never `sleep`). Prefer `Is.Clickable` over a raw `Get`/`FindActive` when you care whether a human could actually perform the action.
2. **Cost.** `data-testid` is a cheap string a build step can strip from production; a beacon is a `MonoBehaviour` shipped in your build, and it only covers objects you decided in advance to beacon. That last point is intentional here (it forces you to enumerate what's worth testing), but it is a real flexibility difference versus a system that can locate anything on screen.

Why the heavier approach is *more* justified in Unity than on the web: the DOM hands you a semantic layer (ARIA roles, labels, accessible names) for free, so role-based locators have rich structure to query. A uGUI Canvas has essentially none — there is no accessibility tree to interrogate — so unless you add an explicit contract yourself, there is nothing stable and semantic to target. The web can lead with `getByRole` precisely because the platform gives it that layer gratis; Unity does not, which is exactly why an explicit, typed beacon is a reasonable *default* in Unity rather than a last resort.

## Requirements

- **Unity 2022.3 LTS or newer.** Needed for `FindObjectsByType` (used internally) and the `asmdef` version-define feature that makes Addressables support optional.
- uGUI (`com.unity.ugui`) and Test Framework (`com.unity.test-framework`), both included with the Editor.
- `asmdef` support: the package is excluded from your actual game build via the Test Assembly checkbox, so no preprocessor directives are littered through the code.
- Addressable Asset System is **optional**. When present, a scene name you pass is treated as either a regular build scene or an AAS key that resolves to the scene.

## How to include

The package lives in the `src/Minefield/Assets/Minefield` subfolder of this repo, so install it from the
Git URL using Unity's `?path=` query. Add this to your project's `Packages/manifest.json`:
...
Pin a specific release by appending `#<tag-or-commit>` after the query, e.g.
`…?path=src/Minefield/Assets/Minefield#v1.0.0`. A Git-URL package does not auto-update; remove its entry
from the `packages-lock.json` `lock` section to refetch the latest.
...
Any Minefield constraint can be inverted with the `!` operator, so you can wait for or assert the
**absence** of a condition — a button that must stay locked mid-transition, an element that must
disappear, and so on:
...
The `!` operator is used instead of `Is.Not` on purpose: Minefield's `Is` inherits from
`NUnit.Framework.Is`, so shadowing `Is.Not` would break ordinary NUnit expressions like
`Is.Not.Null`. `!Is.Clickable` reads just as clearly and cannot collide.

### Timeouts and diagnostics

`WaitUntil` and `ClickWhen` no longer wait forever by default. They fail after
`Beacon.DefaultTimeout` seconds (30s by default, measured in unscaled time), and — crucially — the
failure message explains *why* the constraint never passed. For `Is.Clickable` this tells you
whether the beacon was found, whether it was non-interactable, whether the raycast hit nothing, or
whether something else was drawn on top and swallowed the click.
...
// Change the global default, or disable timeouts entirely to restore the old wait-forever behaviour.
Beacon.DefaultTimeout = 15f;
Beacon.DefaultTimeout = float.PositiveInfinity;
```

This only affects `WaitUntil`/`ClickWhen`. The "play button" pattern (`[NoTimeout]` +
`Utility.WaitForever()`) is unaffected.

### How beacons are found (registry)

Beacons register themselves into an internal lookup while their GameObject is active (via
`OnEnable`/`OnDisable`), so `Beacon.Get`, `FindActive`, and every per-frame constraint check are an
O(1) dictionary lookup instead of a full-scene scan. Two practical consequences: it is much cheaper
inside tight `WaitUntil` loops, and "active" now follows the component's enable state precisely — a
beacon on an inactive GameObject (or a disabled beacon component) is simply not findable, which is
the intended contract. If you subclass a beacon and override `OnEnable`/`OnDisable`, remember to call
`base` or the beacon will not register.
