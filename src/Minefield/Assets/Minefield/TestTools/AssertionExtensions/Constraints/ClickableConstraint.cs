...
namespace E7.Minefield
{
    ...
    public class ClickableConstraint : BeaconConstraint
    {
        ...
        public override string Diagnostic()
        {
            if (!FindResult)
            {
                return $"Beacon '{beaconRequested}' was NOT found active in the scene — the beacon is missing, or its GameObject (or a parent) is inactive.";
            }
            if (!(FoundBeacon is IHandlerBeacon inb))
            {
                return $"Beacon '{beaconRequested}' is a LabelBeacon, not a HandlerBeacon/NavigationBeacon, so it has no clickable area.";
            }

            GameObject firstHit = Utility.RaycastFirst(inb.ScreenClickPoint);
            var selectable = FoundBeacon.GameObject.GetComponent<Selectable>();
            bool interactable = (selectable == null || selectable.IsInteractable());
            bool bubblesToBeacon =
                ReferenceEquals(FoundBeacon.GameObject, ExecuteEvents.GetEventHandler<IPointerClickHandler>(firstHit)) ||
                ReferenceEquals(FoundBeacon.GameObject, ExecuteEvents.GetEventHandler<IPointerDownHandler>(firstHit)) ||
                ReferenceEquals(FoundBeacon.GameObject, ExecuteEvents.GetEventHandler<IPointerUpHandler>(firstHit));

            if (!interactable)
            {
                return $"Beacon '{beaconRequested}' on '{FoundBeacon.GameObject.name}' has a Selectable that is not interactable (Selectable.interactable is false, or a parent CanvasGroup has interactable=false).";
            }
            if (firstHit == null)
            {
                return $"A raycast at the click point of '{beaconRequested}' hit nothing — the target has no active raycast receiver (Graphic.raycastTarget=false, a CanvasGroup with blocksRaycasts=false, no EventSystem in the scene, or the point is off-screen).";
            }
            if (!bubblesToBeacon)
            {
                return $"A raycast at the click point of '{beaconRequested}' hit '{firstHit.name}' first, and the pointer event does not bubble up to this beacon — something is drawn on top of it, or the beacon's object has no pointer handler.";
            }
            return $"Beacon '{beaconRequested}' currently passes the raycast and interactable checks — it was most likely caught in a transient frame (mid-transition) and simply needs more time.";
        }
    }
}