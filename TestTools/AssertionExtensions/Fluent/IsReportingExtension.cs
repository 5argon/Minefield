...
public partial class Is
{
    ...
    {
        /// <summary>
        /// Check out all <see cref="IMinefieldOnOffReporter"/> on **all** components on the object.
        ...
        public static StringContainsConstraint StringContaining(string expectedSubstring) =>
            new StringContainsConstraint(expectedSubstring);

        public static ObjectConstraint<T> Object<T>(T expectedObject) => new ObjectConstraint<T>(expectedObject);

        public static StatusConstraint<T> Status<T>(T expectedStatus) where T : Enum =>
            new StatusConstraint<T>(expectedStatus);
    }
}